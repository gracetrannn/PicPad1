...
open class Brush {
    ...
    open var pointSizeMax: CGFloat = 4
    open var pointSizeMin: CGFloat = 4
    ...
    open func makeLine(from: Pan, to: Pan, hardness: CGFloat) -> [MLLine] {
        let endForce = from.force * 0.95 + to.force * 0.05
        let forceRate = pow(endForce, forceSensitive)
        return makeLine(from: from.point, to: to.point, force: forceRate, hardness: hardness)
    }
    ...
    open func makeLine(from: CGPoint, to: CGPoint, force: CGFloat? = nil, hardness: CGFloat, uniqueColor: Bool = false) -> [MLLine] {
        let force = force ?? forceOnTap
        let scale: CGFloat = 1.0
        let pointRangeDelta = (pointSizeMax - pointSizeMin)
        let pointSize = pointSizeMin + (pointRangeDelta * force)
